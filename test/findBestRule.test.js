const { findBestRule } = require('../src/utils');

describe('findBestRule', () => {
  test('returns null when multiple rules match (ambiguous)', () => {
    const rules = [{ pattern: 'market', group: 'M' }, { pattern: 's-market', group: 'S' }];
  ...
  test('returns single unambiguous match', () => {
    const rules = [{ pattern: 'unique', group: 'U' }, { pattern: 'other', group: 'O' }];
  ...
  test('ignores rules without pattern', () => {
    const rules = [{ noPattern: true }, { pattern: '', group: 'X' }, { pattern: 'shop', group: 'Shop' }];
