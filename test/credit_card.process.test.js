...
test('processCreditCardRecords maps expenses to ready when rule auto', () => {
  const records = [
    { dateRaw: '2026-01-02', merchantRaw: 'Foo Store', amountRaw: -12.34 }
  ];

  const rules = [ { merchant: 'foo', group: 'Groceries', category: 'Food', mode: 'auto', pattern: 'foo' } ];
  ...
  const opts = {
    ...
    findBestRule: (merchantLower, rulesList) => rulesList.find(r => merchantLower.includes(r.pattern)),
