...
// Exported wrapper: catches WorkflowError and returns structured response
function approveMerchantStagingEntries(testSpreadsheetId) {
  const { handleWorkflowErrors } = require("../../core/error_handler");
  return handleWorkflowErrors('MerchantApproval', {}, () => {
    _approveMerchantStagingEntries(testSpreadsheetId);
    return { success: true };
  });
}
