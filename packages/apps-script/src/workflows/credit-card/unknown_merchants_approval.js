 ...
 */
function _approveUnknownMerchants(testSpreadsheetId) {
  ...
  for (let r = 1; r < unknownData.length; r++) {
    ...
    if (mode !== MODE_SKIP) {
      ...
      // Mark row as processing to avoid duplicate side-effects, flush, then create merchant_rules row
      ...
      // For skip mode, mark processing then create rule using provided group/category (can be empty)
      unknownSheet.getRange(rowNum, iStatus + 1).setValue("PROCESSING");
      if (typeof ss.flush === "function") ss.flush();
...
function approveUnknownMerchants(testSpreadsheetId) {
  ...
  return handleWorkflowErrors('UnknownMerchantsApproval', {}, () => {
    _approveUnknownMerchants(testSpreadsheetId);
    return { success: true };
  });
}
