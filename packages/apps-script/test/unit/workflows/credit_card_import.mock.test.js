...
const { adapter } = require("../../e2e/helpers/mocks/tableMock");
...
test.skip("example: parse CSV and write to staging using mock adapter", async () => {
  const csv = fs.readFileSync(
    path.join(__dirname, "fixtures", "credit-card_csvs", "sample.csv"),
    "utf8",
  );
  const mock = adapter;

  // Example usage assuming importer exposes `importCreditCard({ csvText, spreadsheetId, adapter })`
  const importer = require("../src/credit-card/credit-card_import");
