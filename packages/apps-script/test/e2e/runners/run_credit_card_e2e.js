...
const CONFIG = loadConfig({ testBudgetYear: 2026 });
...
async function runUnknownMerchantsApproval() {
  ...
  return await runAppsScriptFunction("approveUnknownMerchants", [testState.testSpreadsheetId]);
}
...
async function validateMerchantApproval() {
  ...
  if (stagingRows.length > 1) {
    errors.push(
      `credit_card_staging should be empty after approval, but has ${
...
async function validateUnknownMerchantsApproval() {
  ...
  if (unknownRows.length > 1) {
    errors.push(
      `unknown_merchants should be empty after approval, but has ${unknownRows.length - 1} entries`,
    );
  } else {
...
async function runCreditCardE2ETests() {
  ...
  try {
    ...
    if (!CONFIG.skipCleanup) {
      await cleanup(drive, [testState.testSpreadsheetId, ...testState.testCsvFiles]);
    } else {
    ...
    if (allErrors.length === 0) {
      ...
      console.log(
        `❌ Credit card E2E tests failed with ${allErrors.length} error(s) (${duration}s)`,
      );
