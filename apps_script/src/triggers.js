...
/* exported installTriggers, removeTriggers, listInstalledTriggers, createTriggerFromConfig, runCreditCardImportWrapper, runDailySummaryWrapper */
function installTriggers() {
  ...
  for (var i = 0; i < SCHEDULE_CONFIG.length; i++) {
    ...
    for (var j = 0; j < existing.length; j++) {
      if (existing[j].getHandlerFunction() === cfg.handler) {
        exists = true;
        break;
      }
    }
...
function removeTriggers() {
  var handlers = SCHEDULE_CONFIG.map(function (c) {
    return c.handler;
  });
...
function runCreditCardImportWrapper() {
  ...
  if (!lock.tryLock(1000 * 60)) {
    Logger.log("runCreditCardImport already running");
    return;
  }
  try {
...
function runDailySummaryWrapper() {
  var lock = LockService.getScriptLock();
  if (!lock.tryLock(1000 * 60)) {
    Logger.log("runDailySummary already running");
    return;
  }
  try {
