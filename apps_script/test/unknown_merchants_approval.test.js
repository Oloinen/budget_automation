...
const HEADERS_UNKNOWN_MERCHANTS = ["merchant","group","category","mode","count","first_seen","last_seen","status"];
...
const approvalCode = fs.readFileSync(
  path.join(__dirname, '../src/credit_card/unknown_merchants_approval.js'), 
  'utf8'
);
...
function loadFunctions() {
  const fn = new Function(
    'SpreadsheetApp', 'normaliseForMatch', 'toIso',
    'STATUS_NEEDS_REVIEW', 'STATUS_APPROVED', 'STATUS_ERROR',
    'MODE_AUTO', 'MODE_REVIEW', 'MODE_SKIP',
    'TAB_MERCHANTS_UNKNOWN', 'TAB_MERCHANT_RULES', 'TAB_CATEGORIES',
    'console',
...
describe('approveUnknownMerchants', () => {
  ...
  test('processes multiple merchants in single run', () => {
    // Valid merchant
    unknownSheet.appendRow([
      "Store1", "Food", "Groceries", MODE_AUTO, 1, "2026-01-05", "2026-01-05", STATUS_NEEDS_REVIEW
    ]);
    // Another valid merchant
    unknownSheet.appendRow([
      "Store2", "Transport", "Gas", MODE_REVIEW, 2, "2026-01-05", "2026-01-06", STATUS_NEEDS_REVIEW
    ]);
    // Invalid merchant
    unknownSheet.appendRow([
      "Store3", "Bad", "Category", MODE_AUTO, 1, "2026-01-07", "2026-01-07", STATUS_NEEDS_REVIEW
    ]);
