...
const HEADERS_UNKNOWN_ITEMS = ["pattern","group","category","mode","count","first_seen","last_seen","status"];
...
const approvalCode = fs.readFileSync(
  path.join(__dirname, '../src/receipts/unknown_items_approval.js'), 
  'utf8'
);
...
function loadFunctions() {
  const fn = new Function(
    'SpreadsheetApp', 'normaliseForMatch',
    'STATUS_NEEDS_REVIEW', 'STATUS_APPROVED', 'STATUS_ERROR',
    'MODE_AUTO', 'MODE_REVIEW', 'MODE_SKIP',
    'TAB_UNKNOWN_ITEMS', 'TAB_ITEM_RULES', 'TAB_CATEGORIES',
    'console',
...
describe('approveUnknownItems', () => {
  ...
  test('processes multiple entries in single run', () => {
    ...
    unknownSheet.appendRow([
      "eggs", "Food", "Groceries", "auto", 2, "2026-01-01", "2026-01-05", STATUS_NEEDS_REVIEW
    ]);
    // Valid entry with review mode
    unknownSheet.appendRow([
      "premium cheese", "Food", "Groceries", "review", 1, "2026-01-05", "2026-01-05", STATUS_NEEDS_REVIEW
    ]);
    // Invalid entry
    unknownSheet.appendRow([
      "unknown", "Bad", "Category", "auto", 1, "2026-01-05", "2026-01-05", STATUS_NEEDS_REVIEW
    ]);
