...
beforeEach(() => {
  // Reset mocks
  mockNormaliseForMatch = jest.fn((s) => String(s || "").toLowerCase().trim());
...
describe('unknowns_utils', () => {
  describe('loadUnknownsIndex', () => {
    it('loads existing rows into an index with normalization', () => {
      const mockSheet = {
        ...
        getRange: jest.fn(() => ({
          getValues: () => [
            ['Amazon', 'Shopping', 'Online', 'auto', 5, '2025-01-01', '2025-01-05', STATUS_APPROVED],
            ['Walmart', 'Groceries', 'Food', '', 2, '2025-01-03', '2025-01-04', STATUS_NEEDS_REVIEW]
          ]
        }))
      };
    ...
    it('skips empty rows', () => {
      ...
      const colMap = { merchant: 1, group: 2, category: 3, mode: 4, count: 5, first_seen: 6, last_seen: 7, status: 8 };
      const idx = loadUnknownsIndex(mockSheet, colMap, 'merchant', true);
  ...
  describe('flushUnknowns', () => {
    it('updates existing rows and appends new rows', () => {
      ...
      expect(mockAppendRows).toHaveBeenCalledWith(mockSheet, expect.arrayContaining([
        expect.any(Array)
      ]));
